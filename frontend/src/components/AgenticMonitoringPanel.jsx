...
function feedbackStatusLabel(value) {
  if (value === 'ok') return 'LLM feedback'
  if (value === 'disabled') return 'Rule-based feedback'
  if (value === 'error') return 'Rule-based feedback'
  return 'Rescan feedback'
}
...
export default function AgenticMonitoringPanel({ result, onResultUpdate }) {
  ...
  const feedback = rescanResult?.feedback
  const ruleBasedFeedback = feedback?.ruleBased
  const llmFeedback = feedback?.llm
    ...
    <div className="agentic-monitoring-panel" data-testid="agentic-monitoring-panel">
      ...
      {rescanResult && (
        <div className="agentic-rescan-result">
          ...
          {feedback && (
            <div className="agentic-feedback">
              <div className="agentic-feedback-head">
                <div>
                  <div className="agentic-card-label">Feedback</div>
                  <strong>{feedbackStatusLabel(llmFeedback?.status)}</strong>
                </div>
                {ruleBasedFeedback?.affectedArtifacts?.length > 0 && (
                  <span className="summary-pill">
                    {ruleBasedFeedback.affectedArtifacts.length} affected artifact(s)
                  </span>
                )}
              </div>
              <p>{feedback.message || ruleBasedFeedback?.summary}</p>
              {llmFeedback?.status === 'disabled' && (
                <small>LLM advisory skipped: {llmFeedback.reason}</small>
              )}
              {llmFeedback?.status === 'error' && (
                <small>LLM advisory unavailable: {llmFeedback.reason}</small>
              )}
              {llmFeedback?.status === 'ok' && llmFeedback.model && (
                <small>Generated by {llmFeedback.model}</small>
              )}
            </div>
          )}
        </div>
